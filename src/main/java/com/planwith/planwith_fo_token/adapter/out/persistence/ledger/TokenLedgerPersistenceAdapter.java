...
public class TokenLedgerPersistenceAdapter implements TokenLedgerPort {
	...
	@Override
	@Transactional
	public TokenLedger save(TokenLedger ledger) {
		if (ledger.tokenLedgerId() != null && !TokenPolicy.ledgerMutable()) {
			throw new IllegalStateException("Ledger UPDATE/DELETE is not allowed.");
		}
		TokenLedgerJpaEntity saved = repository.save(TokenLedgerPersistenceMapper.toEntity(ledger));
		log.debug("TokenLedgerPersistenceAdapter : save : 토큰 원장 INSERT - tokenLedgerUuid={}, type={}",
				ledger.tokenLedgerUuid(), ledger.transactionType());
		return TokenLedgerPersistenceMapper.toDomain(saved);
	}

	@Override
	@Transactional(readOnly = true)
	public Optional<TokenLedger> findByTransactionUuid(TransactionUuid transactionUuid) {
	...
	@Override
	@Transactional(readOnly = true)
	public List<TokenLedger> findByMemberUuidChronological(MemberUuid memberUuid) {
	...
	@Override
	@Transactional(readOnly = true)
	public List<TokenLedger> findByMemberUuidAndEntryType(
